...
`cache loader`是负责检查磁盘上的缓存项和填充缓存元数据到`Nginx`的in-memory数据库。 从本质上讲，`cache loader`准备`Nginx`的实例用来在工作在一个专门分配的目录结构存储的磁盘文件上。它遍历目录，检查缓存内容元数据、更新共享内存中的相关条目，然后退出时准备给下次使用。
